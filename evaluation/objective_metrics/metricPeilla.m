...
% by Piella et al. 
%
% In the original publicaiton (ICIP 2003), the index UIQI is used, rather
% than SSIM. By setting K to very small values, SSIM degards to UIQI.
%In addition, and the window size is 8*8 in the original paper.
window = ones(8);
K = [1e-6 1e-6];

[ssim,ssim_map,sigma1_sq,sigma2_sq]=ssim_index(img1,img2,K,window);
clear ssim, ssim_map;
...
[ssim1,ssim_map1]=ssim_index(fuse,img1,K,window);
[ssim2,ssim_map2]=ssim_index(fuse,img2,K,window);
...
% edge-dependent fusion quality index
    case 3
        ...
        [ssim,ssim_map,sigma1_sq,sigma2_sq]=ssim_index(img1F,img2F,K,window);
        clear ssim, ssim_map;
        ...
        [ssim1,ssim_map1]=ssim_index(fuseF,img1F,K,window);
        [ssim2,ssim_map2]=ssim_index(fuseF,img2F,K,window);
